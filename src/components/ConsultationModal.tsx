    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          exit={{ y: 50, opacity: 0 }}
          className="bg-white rounded-2xl shadow-2xl p-8 md:p-12 w-full max-w-3xl max-h-[90vh] overflow-y-auto"
        ...
        >
          ...
          {isSubmitted ? (
            ...
            <form onSubmit={handleSubmit}>
              <h2 className="text-2xl font-bold text-center mb-4">Book a Free Consultation</h2>
              <div className="mb-8">
                  <div className="relative mt-2">
                      ...
                      <div className="flex justify-between items-center relative">
                          {steps.map(step => (
                              <div key={step.id} className={`text-center ${step.id <= currentStep ? 'text-blue-600' : 'text-gray-400'}`}>
                                  <div className={`w-8 h-8 mx-auto rounded-full border-2 flex items-center justify-center font-bold bg-white ${step.id <= currentStep ? 'border-blue-600' : 'border-gray-300'}`}>
                                      {step.id}
                                  </div>
